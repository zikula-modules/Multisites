 ...
 */
class SiteExtension extends EntityRepository
{
    /**
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('Zikula\MultisitesModule\Entity\SiteExtensionEntity', 'tbl');
        $query = $qb->getQuery();
        ...
        $logger->debug('{app}: User {user} truncated the {entity} entity table.', ['app' => 'ZikulaMultisitesModule', 'user' => UserUtil::getVar('uname'), 'entity' => 'site extension']);
    }
    /**
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('Zikula\MultisitesModule\Entity\SiteExtensionEntity', 'tbl')
        ...
        $logger = $serviceManager->get('logger');
        $logger->debug('{app}: User {user} updated {entities} created by user id {userid}.', ['app' => 'ZikulaMultisitesModule', 'user' => UserUtil::getVar('uname'), 'entities' => 'site extensions', 'userid' => $userId]);
    }
    ...
    public function updateLastEditor($userId, $newUserId)
    {
        ...
        $logger = $serviceManager->get('logger');
        $logger->debug('{app}: User {user} updated {entities} edited by user id {userid}.', ['app' => 'ZikulaMultisitesModule', 'user' => UserUtil::getVar('uname'), 'entities' => 'site extensions', 'userid' => $userId]);
    }
    ...
    public function deleteByCreator($userId)
    {
        ...
        $logger = $serviceManager->get('logger');
        $logger->debug('{app}: User {user} deleted {entities} created by user id {userid}.', ['app' => 'ZikulaMultisitesModule', 'user' => UserUtil::getVar('uname'), 'entities' => 'site extensions', 'userid' => $userId]);
    }
    ...
    public function deleteByLastEditor($userId)
    {
        ...
        $logger = $serviceManager->get('logger');
        $logger->debug('{app}: User {user} deleted {entities} edited by user id {userid}.', ['app' => 'ZikulaMultisitesModule', 'user' => UserUtil::getVar('uname'), 'entities' => 'site extensions', 'userid' => $userId]);
    }
     ...
     *
     * @return array|Zikula\MultisitesModule\Entity\SiteExtensionEntity retrieved data array or Zikula\MultisitesModule\Entity\SiteExtensionEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved Zikula\MultisitesModule\Entity\SiteExtensionEntity instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function selectWherePaginated($where = '', $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true, $slimMode = false)
    {
        ...
        if (!$hasFilters) {
            ...
            if ($page > 1 || isset($_GET['pos'])) {
                ...
                $session->set('ZikulaMultisitesModuleSiteExtensionsCurrentPage', $page);
            } else {
                // restore current page from session
                $page = $session->get('ZikulaMultisitesModuleSiteExtensionsCurrentPage', 1);
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('ZikulaMultisitesModule', 'category', 'buildFilterClauses', ['qb' => $qb, 'ot' => 'siteExtension', 'catids' => $v]);
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('Zikula\MultisitesModule\Entity\SiteExtensionEntity', 'tbl');
