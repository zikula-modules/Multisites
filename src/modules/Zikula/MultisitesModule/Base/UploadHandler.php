...
class UploadHandler
{
        ...
        if ($maxSize > 0) {
            ...
            if ($fileSize > $maxSize) {
                ...
                if ($maxSizeKB < 1024) {
                    ...
                    $logger->error('{app}: User {user} tried to upload a file with a size greater than "{size} KB".', ['app' => 'ZikulaMultisitesModule', 'user' => $this->currentUserApi->get('uname'), 'size' => $maxSizeKB]);
                ...
                $logger->error('{app}: User {user} tried to upload a file with a size greater than "{size} MB".', ['app' => 'ZikulaMultisitesModule', 'user' => $this->currentUserApi->get('uname'), 'size' => $maxSizeMB]);
    ...
    protected function validateFileUpload($objectType, $file, $fieldName)
    {
        ...
        if (!is_array($file) || (is_array($file) && $file['error'] != '0')) {
            ...
            $logger->error('{app}: User {user} tried to upload a file which could not be found.', ['app' => 'ZikulaMultisitesModule', 'user' => $this->currentUserApi->get('uname')]);
    ...
    private function handleError($file)
    {
        ...
        $logger = $serviceManager->get('logger');
        $logger->error('{app}: User {user} received an upload error: "{errorMessage}".', ['app' => 'ZikulaMultisitesModule', 'user' => $this->currentUserApi->get('uname'), 'errorMessage' => $errorMessage]);
