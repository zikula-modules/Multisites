 ...
 */
class Multisites_Form_Handler_Site_Base_Edit extends Multisites_Form_Handler_Common_Edit
{
    /**
     * Pre-initialise hook.
     *
    ...
    public function preInitialize()
    {
        ...
        $this->uploadFields = array('logo' => false, 'favIcon' => false, 'parametersCsvFile' => false);
    ...
    public function initialize(Zikula_Form_View $view)
    {
        ...
        if ($this->mode == 'create') {
            ...
            if (!$modelHelper->canBeCreated($this->objectType)) {
                LogUtil::registerError($this->__('Sorry, but you can not create the site yet as other items are required which must be created before!'));
        ...
        if (!empty($this->relationPresets['template'])) {
            $relObj = ModUtil::apiFunc($this->name, 'selection', 'getEntity', array('ot' => 'template', 'id' => $this->relationPresets['template']));
            if ($relObj != null) {
        ...
        if (!empty($this->relationPresets['project'])) {
            $relObj = ModUtil::apiFunc($this->name, 'selection', 'getEntity', array('ot' => 'project', 'id' => $this->relationPresets['project']));
            if ($relObj != null) {
                $relObj->addSites($entity);
            }
        }
    ...
    protected function getRedirectUrl($args)
    {
        ...
        switch ($this->returnTo) {
            ...
            case 'adminViewTemplate':
                return ModUtil::url($this->name, 'admin', 'view', array('ot' => 'template'));
            case 'adminViewProject':
                return ModUtil::url($this->name, 'admin', 'view', array('ot' => 'project'));
            default:
                return $this->getDefaultReturnUrl($args);
        }
    }
