 ...
 *
 * This is the base validation class for template entities.
 */
class Multisites_Entity_Validator_Base_Template extends Multisites_Validator
{
    /**
    ...
    public function validateAll()
    {
        ...
        if (!$this->isStringNotLongerThan('description', 250)) {
            $errorInfo['message'] = __f('Error! Length of field value must not be higher than %2$s (%1$s).', array('description', 250), $dom);
            return $errorInfo;
        }
        if (!$this->isStringNotLongerThan('sqlFile', 255)) {
            $errorInfo['message'] = __f('Error! Length of field value must not be higher than %2$s (%1$s).', array('sql file', 255), $dom);
            return $errorInfo;
        }
        if (!$this->isStringNotEmpty('sqlFile')) {
            $errorInfo['message'] = __f('Error! Field value must not be empty (%s).', array('sql file'), $dom);
            return $errorInfo;
        }
     ...
     *
     * This method determines if there already exist templates with the same template.
     *
