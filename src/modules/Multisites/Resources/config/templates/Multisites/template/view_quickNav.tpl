...
{assign var='objectType' value='template'}
<form action="{$modvars.ZConfig.entrypoint|default:'index.php'}" method="get" id="multisitesTemplateQuickNavForm" class="multisites-quicknav">
    <fieldset>
        ...
        <input type="hidden" name="ot" value="template" />
        ...
        {if !isset($projectFilter) || $projectFilter eq true}
                <label for="projects">{gt text='Project'}</label>
                {php}
                    $mainSearchTerm = '';
                    if (isset($_GET['q'])) {
                ...
                {foreach item='option' from=$listEntries}
                    ...
                    <option value="{$entryId}"{if $entryId eq $projects} selected="selected"{/if}>{$option->getTitleFromDisplayPattern()}</option>
                {/foreach}
                </select>
                {php}
        ...
        {if !isset($sorting) || $sorting eq true}
                ...
                <select id="sortBy" name="sort">
                    ...
                    <option value="sqlFile"{if $sort eq 'sqlFile'} selected="selected"{/if}>{gt text='Sql file'}</option>
                    <option value="folders"{if $sort eq 'folders'} selected="selected"{/if}>{gt text='Folders'}</option>
                    <option value="excludedTables"{if $sort eq 'excludedTables'} selected="selected"{/if}>{gt text='Excluded tables'}</option>
...
/* <![CDATA[ */
    document.observe('dom:loaded', function() {
        multisitesInitQuickNavigation('template');
        if ($('projects') != undefined) {
            $('projects').observe('change', function () { multisitesSubmitQuickNavForm('template'); });
        }
