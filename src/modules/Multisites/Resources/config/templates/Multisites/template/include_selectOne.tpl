...
{/if}
    ...
    {if $group eq 'site' && $mandatory eq false}
        <p class="z-informationmsg">{gt text='If you decouple a site from the template it is not affected by future reapplications of this template anymore, but is configured and maintained independently. If you reassign a decoupled site to a template again this template is reapplied for that site.'}</p>
    {/if}
{if isset($panel) && $panel eq true}
    </fieldset>
{else}
