    ...
    {/if}
        <table class="z-datatable">
            ...
            <tr>
                ...
                <th id="hName" scope="col" class="z-left">
                    {sortlink __linktext='Name' currentsort=$sort modname='Multisites' type=$lct func='view' sort='name' sortdir=$sdir all=$all own=$own workflowState=$workflowState searchterm=$searchterm pageSize=$pageSize ot='project'}
                </th>
                <th id="hTemplates" scope="col" class="z-left">
                    {gt text='Templates'}
                </th>
        ...
        {foreach item='project' from=$items}
            <tr class="{cycle values='z-odd, z-even'}">
                ...
                <td headers="hTemplates" class="z-left">
                    {if count($project.templates) gt 0}
                        <ul>
                        {foreach item='template' from=$project.templates}
                            <li>{$template->getTitleFromDisplayPattern()}</li>
                        {/foreach}
                        </ul>
                    {else}
                        {gt text='None'}
                ...
                <td headers="hSites" class="z-left">
                    <a href="{modurl modname='Multisites' type='admin' func='view' ot='site' project=$project.id}" title="{gt text='View sites assigned to this project'}">{gt text='Site list'}</a>
                </td>
