...
class Multisites_Controller_InteractiveInstaller extends Zikula_Controller_AbstractInteractiveInstaller
{
    ...
    public function install()
    {
        ...
        return $this->view->fetch('Multisites_admin_init.tpl');
    }
     ...
     * @return post the physical folder path
     */
    public function step2($args)
    {
        ...
        $files_real_path = isset($args['files_real_path']) ? $args['files_real_path'] : $this->request->query->get('files_real_path', null);
    ...
    public function step21($args)
    {
        ...
        $files_real_path = isset($args['files_real_path']) ? $args['files_real_path'] : $this->request->request->get('files_real_path', null);
        if ($files_real_path == '') {
        ...
        if (!file_exists($files_real_path)) {
            ...
            return $this->redirect(ModUtil::url($this->name, 'interactiveinstaller', 'step2', array('files_real_path' => $files_real_path)));
        }
        ...
        if ($fh == false) {
            ...
            return $this->redirect(ModUtil::url($this->name, 'interactiveinstaller', 'step1'));
        }
        $lines = file($file);
        $final_file = '';
        foreach ($lines as $line_num => $line) {
            if (strpos($line, "ZConfig['Multisites']['multisites.files_real_path']")) {
                $line =  str_replace('$files_real_path', $files_real_path, $line);
            }
            $final_file .= $line;
        }
        ...
        return $this->redirect(ModUtil::url($this->name, 'interactiveinstaller', 'step3'));
    }
    ...
    public function step31($args)
    {
        ...
        $mainsiteurl = isset($args['mainsiteurl']) ? $args['mainsiteurl'] : $this->request->request->get('mainsiteurl', null);
        $sitednsEndText = isset($args['sitednsEndText']) ? $args['sitednsEndText'] : $this->request->request->get('sitednsEndText', null);
        $site_temp_files_folder = isset($args['site_temp_files_folder']) ? $args['site_temp_files_folder'] : $this->request->request->get('site_temp_files_folder', null);
        $site_files_folder = isset($args['site_files_folder']) ? $args['site_files_folder'] : $this->request->request->get('site_files_folder', null);
        ...
        return $this->redirect(ModUtil::url($this->name, 'interactiveinstaller', 'step4'));
    }
    ...
    public function step4()
    {
        ...
        $path = 'config/multisites_config.php';
        $fileWriteable = (is_writeable($path)) ? true : false;
        ModUtil::load('Extensions', 'admin');
