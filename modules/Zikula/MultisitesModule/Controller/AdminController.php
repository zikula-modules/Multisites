...
class AdminController extends AbstractController
{
    ...
    protected function isMainSite(Request $request)
    {
        ...
        $isBasedOnDomains = isset($msConfig['based_on_domains']) && '~' !== $msConfig['based_on_domains']
            ? $msConfig['based_on_domains']
            : 1
        ;
        $mainUrl = isset($msConfig['mainsiteurl']) && '~' !== $msConfig['mainsiteurl']
            ? $msConfig['mainsiteurl']
            : ''
        ;

        return (0 == $isBasedOnDomains && $mainUrl == $request->query->get('sitedns', '')
            || (1 == $isBasedOnDomains && $mainUrl == $_SERVER['HTTP_HOST']));
    }
    ...
    public function multiplyQueriesAction(Request $request)
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            if (
                empty($sqlInput)
                && isset($_FILES['queryfile'])
                && is_array($_FILES['queryfile'])
                && isset($_FILES['queryfile']['tmp_name'])
                && is_file($_FILES['queryfile']['tmp_name'])
                && is_readable($_FILES['queryfile']['tmp_name'])
            ) {
            ...
            if ($inputValid === true) {
                ...
                foreach ($databases as $database) {
                    ...
                    if ('show' === $opMode) {
                        ...
                        $sqlOutput .= "\n\n";
                        $sqlOutput .= '# ------ '
                            . $this->__('Site alias') . ': ' . $database['alias'] . ', '
                            . $this->__('Database name') . ': ' . $dbName . "\n\n"
                            . $sql . "\n\n"
                        ;
                    } elseif ('execute' === $opMode) {
                        // run sql in site database
                        $connect = $systemHelper->connectToExternalDatabase(new DatabaseInfo($database));
                        if (!$connect) {
                            $this->addFlash(
                                'error',
                                $this->__f('Error! Connecting to the database %s failed.', ['%s' => $dbName])
                            );
                            continue;
                        }
                        ...
                        if (!$stmt->execute()) {
                            $this->addFlash(
                                'error',
                                $this->__f('Error during executing query in database %s.', ['%s' => $dbName])
                            );
                        } else {
                            $this->addFlash(
                                'status',
                                $this->__f('Query successfully executed in database %s.', ['%s' => $dbName])
                            );
                        }
                    }
                }
